...
function parseLine(line,type,banks,vy,vm){
  ...
  if(dateMatch){
    const d=String(dateMatch[1]).padStart(2,"0"),m=String(dateMatch[2]).padStart(2,"0");
    const y=dateMatch[3]?(dateMatch[3].length===2?"20"+dateMatch[3]:dateMatch[3]):vy;
    date=`${y}-${m}-${d}`;
  }
  ...
  if(type==="expense"){
    ...
    for(const b of banks){
      if(rem.toLowerCase().includes(b.name.toLowerCase())){bank=b.name;rem=rem.replace(new RegExp(b.name,"gi"),"").replace(/\s+/g," ").trim();break;}
    }
    const mm={pix:"PIX",crédito:"Crédito",credito:"Crédito",débito:"Débito",debito:"Débito",dinheiro:"Dinheiro",boleto:"Boleto"};
...
function Modal({onClose,children,tall=false}){
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.87)",backdropFilter:"blur(6px)",zIndex:1000,display:"flex",alignItems:"flex-end",justifyContent:"center"}}
      onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{background:"var(--card)",border:"1px solid var(--border2)",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:520,padding:"20px 18px 36px",maxHeight:tall?"95vh":"88vh",overflowY:"auto"}}
          ...
          <div className="pg">
            ...
            <div className="row2">
              ...
              <div className="card">
                ...
                <Donut size={120} thick={21} data={bankCredit.filter(b=>b.spent>0).map(b=>({color:b.color,value:b.spent}))} label={fmt(bankCredit.reduce((s,b)=>s+b.spent,0))} sublabel="crédito"/>
            ...
            {catData.length>0&&(
              <div className="card">
                <div className="st" style={{marginBottom:10}}>Gastos por categoria</div>
                {catData.map(c=>{
